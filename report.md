...
| Router | 25/25 (100.0%) | 0.970 | 3.0 | 2.9s |
...
Router 的设计目标是将简单步骤路由到本地小模型（Qwen2.5-1.5B Q4_K_M，llama.cpp 推理），复杂步骤路由到 API 大模型（DeepSeek-V3），在保证质量的同时降低 API 调用成本。

**路由规则**：三步判断——可选动作数（>5 → API）、历史步数（>3 → API）、循环检测（连续重复 ≥2 → API）。

**AlfWorld 结果**：Router 与纯 API 性能一致（100%、5.8 steps），但本地模型几乎未被使用。Qwen2.5-1.5B 在 AlfWorld 的结构化动作空间（10 种动作、精确格式要求）中频繁出错：生成 "put X on Y" 而非 "put X in Y"、混淆当前位置、跳过必要步骤。结果，Router 的保守阈值将 95%+ 的步骤路由到了 API，本地模型形同虚设。

**WebShop 结果**：Router 展现出实际价值。每个 3 步任务（search → click → buy）中，第一步（search）始终路由到本地模型，后两步（click → buy）走 API。原因在于第一步只有一种动作（search），任务仅仅是提取目标中的关键词——Qwen2.5-1.5B 完全能胜任。最终 25 个任务全部成功，奖励与纯 API 一致（0.970），节省约 33% 的 API 调用，仅增加 0.3s 延迟。

**关键发现**：Router 快慢双系统的可行性取决于"简单步骤"的粒度。在动作空间单一、推理需求低的步骤（如 WebShop 搜索），1.5B 模型完全够用；在需要精确格式遵循和空间推理的步骤（如 AlfWorld 物体操作），小模型力不从心。实际部署中，应按任务类型而非统一阈值进行路由决策。
...
5. **Router 在简单步骤上可行，小模型能力是瓶颈**：WebShop 第一步搜索可由 Qwen2.5-1.5B 可靠处理，节省 33% API 调用；AlfWorld 的结构化动作格式超出小模型能力，路由退化为全 API。按任务粒度路由比统一阈值更有效。

### 6.2 局限与展望

- **Router 受限于小模型能力**：Qwen2.5-1.5B 在 AlfWorld 上不足以处理结构化动作，换用 3B–7B 模型或微调后的模型可能改善路由效果。
